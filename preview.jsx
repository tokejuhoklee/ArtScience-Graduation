...
function expandBlock(blk, G, pos0) {
  ...
  const move = (from, to) => {
    const dt = moveTime(Math.abs(to - from), spd, brk, soft);
    ev.push({ type:"move", t0:t, t1:t+dt, p0:from, p1:to, brk, soft, id });
    pos = to; t += dt;
  };
  const wait = (ms) => {
    if (ms <= 0) return;
  ...
  const home = () => {
    const h = Math.round(pos/360)*360;
    if (Math.abs(pos - h) > 1) move(pos, h);
  };

  const rtn = G.returnToNeutral !== false;
  ...
  if (blk.type === "pendulum") {
    if (blk.oneSided) {
      for (let i = 0; i < blk.reps; i++) {
        move(pos,  ang); wait(rest);
        move(pos,    0); wait(rest);
      }
    } else {
      for (let i = 0; i < blk.reps; i++) {
        move(pos,  ang); wait(rest);
...
function compileToSteps(blocks, G) {
  ...
  for (const blk of blocks) {
    ...
    if (blk.type === "pendulum") {
      ...
      out.push(["speed", spd], ["angle", ang]);
      if (blk.oneSided) {
        for (let i = 0; i < blk.reps; i++) {
          out.push(["left"],              ["wait_pend"]);
          if (rest > 0) out.push(["wait", rest]);
          out.push(["offset", -angSteps], ["wait_pend"]);
          if (rest > 0) out.push(["wait", rest]);
        }
      } else {
        for (let i = 0; i < blk.reps; i++) {
          out.push(["left"],  ["wait_pend"]);
          if (rest > 0) out.push(["wait", rest]);
          out.push(["right"], ["wait_pend"]);
          if (rest > 0) out.push(["wait", rest]);
        }
      }
      if (rtn) out.push(["move", 0], ["wait_pend"]);
      ...
      out.push(["speed", sSpd], ["angle", sAng]);
      for (let i = 0; i < smallReps; i++) {
        out.push(["left"],  ["wait_pend"]);
      ...
      if (rtn) out.push(["speed", spd], ["move", 0], ["wait_pend"]);
      else     out.push(["speed", spd]);

    } else if (blk.type === "spin") {
      out.push(["speed", spd]);
      if (blk.continuous) {
        const dirs = blk.dir === "both" ? ["cw","ccw"] : [blk.dir > 0 ? "cw" : "ccw"];
    ...
    <div onClick={onSelect} style={{borderRadius:8,border:`1px solid ${selected?col:"#1c1c2c"}`,
      ...
      {selected && (
        <div onClick={e=>e.stopPropagation()}
          ...
          {blk.type==="pendulum"&&
            <Tog label="One-sided (0 → +ang → 0)" on={!!blk.oneSided} set={v=>upd({oneSided:v})}/>}
          ...
          <div style={{paddingTop:7,borderTop:"1px solid #161626",display:"flex",flexDirection:"column",gap:7}}>
            ...
            {blk.speed!=null&&<Slider label="Speed" value={blk.speed} min={100} max={8000} step={100}
...
export default function App() {
  ...
  const [returnToNeutral, setReturnToNeutral] = useState(true);
  const G = { speed, angle, restMs, brk, soft, angleMult, restMult, returnToNeutral };
  ...
  useEffect(()=>{
    ...
    startAnim(s);
  }, [blocks, speed, angle, restMs, brk, soft, angleMult, restMult, returnToNeutral]);
        ...
        <div style={{flex:1,overflowY:"auto",padding:11,display:"flex",flexDirection:"column",gap:10}}>
          ...
          {tab==="compose"&&<>
            ...
            <div style={card}>
              ...
              <Slider label="Speed" value={speed} min={100} max={8000} step={100}
              ...
              <div style={{borderTop:`1px solid ${BDR}`,paddingTop:8,display:"flex",flexDirection:"column",gap:6}}>
                ...
                <Tog label="Return to neutral" on={returnToNeutral} set={setReturnToNeutral}/>
              </div>
            </div>
