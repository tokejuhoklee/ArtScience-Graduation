/*
  Pico W 2 — JSS57 Stepper Web Controller (24V + GEAR REDUCTION VERSION)
  - Optimized for 24V power supply (~1200 RPM motor max)
  - Gear reduction support: 5:1 gearbox for accurate angle control
  ...
  - NOW WITH HARDWARE TIMER for smooth, jitter-free stepping!
*/
...
#include "hardware/timer.h"  // ← ADDED for hardware timer

//
// === HARDWARE CONFIG ===
//
bool useSerialControl = true;  // Enable serial commands
String serialBuffer = "";
...
int drivingTeeth = 1;     // Motor side (input)
int drivenTeeth = 5;      // Output side (5:1 reduction)
float gearReduction = 5.0; // Calculated: drivenTeeth / drivingTeeth
...
// At 36V, motor can reach full 1800 RPM = 12000 steps/sec
// For safety and torque, max recommended is 8000-10000 steps/sec
int currentSpeed = 2000;             // default cruise speed
int minSpeed = 50, maxSpeed = 10000;  // Increased for 36V
...
void updateMoveRangeFromAngle() {
  ...
  // Info message about speeds for different output RPMs with 5:1 gearbox
  if (currentAngle == 90 || currentAngle == 180) {
    appendLog("ℹ️  36V speeds: 2000=60rpm, 4000=120rpm, 6000=180rpm, 8000=240rpm (output shaft)");
  }
}
...
void handleCmd() {
  ...
  if (cmd == "off") { setMotorEnabled(false); pendulumMode=false; continuousMode=false; server.send(200, "text/plain", "OK"); return; }
  if (cmd == "stop") { emergencyStop(); continuousMode=false; server.send(200, "text/plain", "OK"); return; }
  ...
  if (cmd.startsWith("speed ")) {
    ...
    // Calculate and display output RPM for reference
    float motorRPM = (currentSpeed / 400.0) * 60.0;
    float outputRPM = motorRPM / gearReduction;
    appendLog("Speed: " + String(currentSpeed) + " sps (motor: " + String((int)motorRPM) + 
              " rpm, output: " + String((int)outputRPM) + " rpm)");
    server.send(200,"text/plain","OK"); return;
  }
  ...
  // Gear command: "gear 1:5" for 5:1 gearbox
  if (cmd.startsWith("gear ")) {
    ...
    appendLog("Invalid gear format. Use: gear 1:5");
  ...
  if (cmd == "config") {
    appendLog("=== CONFIG ===");
    appendLog("Power: 36V (motor can reach full 1800 RPM)");
    ...
    float motorRPM = (currentSpeed / 400.0) * 60.0;
    float outputRPM = motorRPM / gearReduction;
    appendLog("speed: " + String(currentSpeed) + " sps (" + String((int)motorRPM) + 
              " motor rpm, " + String((int)outputRPM) + " output rpm)");
...
//
// === STEPPER TICK (now called from hardware timer interrupt) ===
//
void stepperTick() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n' || c == '\r') {
      if (serialBuffer.length() > 0) {
        ...
        // Process command (reuse your existing command logic)
        appendLog("> SERIAL: " + serialBuffer);
        
        String cmd = serialBuffer;
        
        // Copy-paste your command handling from handleCmd()
        if (cmd == "on") { setMotorEnabled(true); }
        else if (cmd == "off") { setMotorEnabled(false); pendulumMode=false; continuousMode=false; }
        else if (cmd == "stop") { emergencyStop(); continuousMode=false; }
        else if (cmd == "left") {
          if (motorEnabled) { currentTarget = moveRange; smartMoveTo(currentTarget); }
        }
        else if (cmd == "right") {
          if (motorEnabled) { currentTarget = -moveRange; smartMoveTo(currentTarget); }
        }
        else if (cmd == "neutral") {
          if (motorEnabled) { smartMoveTo(0); }
        }
        else if (cmd == "pend") {
          if (motorEnabled) {
            pendulumMode = !pendulumMode;
          ...
          int v = cmd.substring(6).toInt();
          currentSpeed = constrain(v, minSpeed, maxSpeed);
          appendLog("Speed: " + String(currentSpeed) + " sps");
        }
        ...
        else if (cmd == "brakeon") { enableBraking = true; appendLog("Braking ON"); }
        else if (cmd == "brakeoff") { enableBraking = false; appendLog("Braking OFF"); }
        else if (cmd == "softstarton") { enableSoftStart = true; appendLog("Soft-start ON"); }
        else if (cmd == "softstartoff") { enableSoftStart = false; appendLog("Soft-start OFF"); }
        else if (cmd == "config") {
          appendLog("=== CONFIG ===");
          appendLog("Gear: " + String(drivingTeeth) + ":" + String(drivenTeeth));
          ...
          appendLog("Speed: " + String(currentSpeed) + " sps");
          appendLog("Braking: " + String(enableBraking ? "ON" : "OFF"));
          appendLog("Motor: " + String(motorEnabled ? "ON" : "OFF"));
        }
        else if (cmd == "help") {
          appendLog("Commands: on, off, stop, left, right, neutral, pend, cw, ccw");
          appendLog("  speed <n>, angle <n>, delay <n>, gear <d>:<r>");
          appendLog("  brakeon, brakeoff, softstarton, softstartoff, config");
        }
        else {
...
void setup() {
  Serial.begin(115200);
  delay(200);
  appendLog("=== PicoW Stepper Controller (36V + 5:1 Gear + HW Timer) ===");

  setupPins();
  updateGearReduction();
  updateMoveRangeFromAngle();

  // Start hardware timer for step generation (runs every 50 microseconds)
  // This gives us up to 20kHz step rate capability with minimal jitter
  add_repeating_timer_us(50, step_timer_callback, NULL, &step_timer);
  appendLog("✓ Hardware timer started for smooth stepping");
  ...
  appendLog("✓ Running at 36V - full 1800 RPM capability!");
}

void loop() {
  
 // Handle serial commands from USB
  if (useSerialControl) {
    handleSerialCommands();
  }
  
  // Network servicing
  static unsigned long lastNetworkService = 0;
  if (millis() - lastNetworkService >= 5) {
  ...
  if (pendulumMode && motorEnabled) {
    ...
    if (atTarget && localStepsRemaining == 0) {  // Use local copy
      if (!positionJustReached) {
        positionReachedTime = millis();
        positionJustReached = true;
        appendLog("Target reached. Position: " + String(localCurrentPosition) + " Target: " + String(currentTarget));
  ...
  // No delay(0) needed - hardware timer handles stepping independently
}
...
<head>
  ...
  <title>Pico Stepper (36V)</title>
  <style>
    :root{--accent:#7c4dff;--bg:#0b0b12;--muted:#a99bd1;--warning:#ff9800}
...
.success{padding:8px;background:#003320;border-left:3px solid #4caf50;border-radius:6px;margin-bottom:8px;font-size:12px}
.gear-section{background:#1a1a2e;padding:10px;border-radius:6px;margin:10px 0}
.speed-ref{font-size:10px;color:#666;margin-top:4px}
</style>
</head>
<body>
  <div class="wrap">
    <header><h1>⚙️ Pico Stepper (36V + HW Timer)</h1></header>
    <div class="card">
  <div class="success">
    ✓ <strong>36V Mode + Hardware Timer:</strong> Smooth, jitter-free stepping at full 1800 RPM capability!
  </div>
  <div class="status">
    ...
    <div style="width:300px">
      ...
      <div class="gear-section">
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-top:6px">
          <input type="number" id="drivingTeeth" value="1" min="1" max="200">
          <span>:</span>
          <input type="number" id="drivenTeeth" value="5" min="1" max="200">
          <button onclick="updateGear()" style="padding:6px 10px">Set</button>
        </div>
        <div style="font-size:11px;margin-top:4px;color:#888">
          Motor : Output (e.g. 1:5 = 5:1 gearbox reduction)
        </div>
      </div>
      ...
      <div style="margin-top:12px">
        ...
        <label>Speed: <span id="speedVal"></span> sps <span id="rpmDisplay" class="speed-ref"></span></label>
        <input id="speed" type="range" min="50" max="10000" value="2000" oninput="updateSpeed(this.value)">
        <div class="speed-ref">
          At 5:1 reduction: 2000=60rpm, 4000=120rpm, 6000=180rpm, 8000=240rpm, 10000=300rpm (output)
        </div>
        
        <label>Arm Angle: <span id='angleVal'></span>° (output shaft motion)</label>
        ...
        <input id="delay" type="range" min="0" max="5000" step="50" value="500" oninput="updateDelay(this.value)">
        
        <div style="margin-top:8px">
          <label style="display:inline;margin-right:10px">
            <input type="checkbox" id="restToggle" onchange="toggleRest(this.checked)"> Disable at rest
          </label>
          <label style="display:inline;margin-right:10px">
            <input type="checkbox" id="brakeToggle" checked onchange="toggleBrake(this.checked)"> Braking
          </label>
        ...
        <div style="margin-top:8px;font-size:11px;color:#888">
          💡 Soft-start reduces mechanical shock. For whip effect: disable soft-start + braking.<br>
          ⚡ Hardware timer provides smooth, jitter-free stepping even during WiFi activity!
        </div>
  ...
  <footer>
    ...
    <small>GP2=STEP, GP3=DIR, GP4=ENA, GP5=PEND | 5:1 gearbox | 36V power | Hardware timer stepping</small>
  </footer>
</div></div>
<script>
...
function updateSpeed(v){ 
  ...
  // Calculate RPM display (assumes 5:1 reduction)
  let motorRPM = Math.round((v / 400.0) * 60);
  let outputRPM = Math.round(motorRPM / 5.0);
  document.getElementById('rpmDisplay').textContent = '(' + motorRPM + ' motor, ' + outputRPM + ' output rpm)';
  fetch('/cmd?c='+encodeURIComponent('speed '+v)); 
}
